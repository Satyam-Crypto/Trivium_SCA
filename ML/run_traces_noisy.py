...
logfile = open("trace_logs_noisy.log", "a+", buffering=1)
print("\n\nTime: %s\n" % curr_time)
logfile.write("\n\nTime: %s\n" % curr_time)
...
stddev = float(config['ML']['StdDev'])
print("StdDev: ", stddev)
logfile.write("StdDev: %s\n" % stddev)

path = "traces_models_noisy"
...
X_train, X_val, X_test, y_train, y_val, y_test, snr = load_and_split_data(
    train_split, val_split, test_split, datasize, stddev)
print("SNR: ", snr)
logfile.write("SNR: %s \n" % snr)
...
logfile.write("Test Loss - {:.5f}, Test Acc - {:.5f}\n".format(
    test_loss, test_acc))
print("Test Accuracy for 1-4 tolerance: ")
print(test_accs_dict)
logfile.write("Test Accuracy for 1-4 tolerance: \n")
